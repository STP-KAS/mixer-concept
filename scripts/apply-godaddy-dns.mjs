...
if (/sso|signin|login/i.test(page.url())) {
  ...
  try {
    await page.waitForURL((url) => /dcc\.godaddy\.com|dns\.godaddy\.com|dashboard\.godaddy\.com/i.test(String(url)) && !/sso|signin/i.test(String(url)), {timeout: 180000});
  } catch {
...
await page.screenshot({path: join(homedir(), 'AppData/Local/Temp/mix-godaddy-dns.png'), fullPage: true});
await context.close();
