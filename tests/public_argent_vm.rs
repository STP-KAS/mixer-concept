// Executes the exact signed SDK transactions for real Argent-generated closed ICC.
...
fn artifact_value(v:&Value)->ArtifactValue {if let Some(b)=v.as_bool(){ArtifactValue::Bool(b)}else if let Some(n)=v.as_i64(){ArtifactValue::Int(n)}else{ArtifactValue::Bytes(bytes(string(v)))}}
fn compile(v:&Value)->SilAbiArtifact{
 let name=string(&v["contractName"]);let source=std::fs::read_to_string(std::path::Path::new(&std::env::var("KE_ARGENT_CONTRACT_DIR").unwrap()).join(format!("{name}.sil"))).unwrap();
 let fields=if name=="Warden"{vec!["owner","food"]}else{vec!["owner","controller_id","energy"]};let args=fields.iter().map(|f|artifact_value(&v["state"][*f])).collect::<Vec<_>>();
...
fn exact_sdk_transactions_and_adversarial_rejections(){
 ...
 let fixtures=data["fixtures"].as_array().unwrap();assert!(fixtures.len()>=18);
 for fixture in fixtures{
  let name=string(&fixture["name"]);let (tx,entries)=decode(&fixture["transaction"]);
  // Cross-check the JS adapter against the generated SilverScript and native ABI encoder.
  for (i,t) in fixture["calls"].as_array().unwrap().iter().enumerate(){
   if t.is_null(){continue;}
   let a=compile(t);let args=t["args"].as_array().unwrap().iter().map(artifact_value).collect::<Vec<_>>();
   let mut expected=encode_entry_sig_script(&a,string(&t["entry"]),&args).unwrap();expected.extend(push_redeem_script(&bytecode(&a)));
   assert_eq!(tx.inputs[i].signature_script,expected,"Argent ABI bytes: {name} input {i}");
   assert_eq!(entries[i].script_public_key,kaspa_txscript::pay_to_script_hash_script(&bytecode(&a)),"Argent generated template: {name}");
  }
