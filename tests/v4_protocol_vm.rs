...
fn state(v:&Value)->ArtifactValue {v.as_object().unwrap().iter().map(|(k,v)|(k.clone(),artifact_value(v))).collect::<BTreeMap<_,_>>().into()}
fn compile(v:&Value,kind:&str)->SilAbiArtifact{
 let source=std::fs::read_to_string(std::path::Path::new(&std::env::var("KE_CONTRACT_DIR").unwrap()).join(format!("v4-{kind}.sil"))).unwrap();
 let args=v["fields"].as_array().unwrap().iter().map(|f|artifact_value(&v["state"][string(&f["name"])] )).collect::<Vec<_>>();
...
fn exact_sdk_transactions_and_adversarial_rejections(){
 let data:Value=serde_json::from_str(&std::fs::read_to_string(std::env::var("KE_V4_FIXTURES").unwrap()).unwrap()).unwrap();
 assert_eq!(data["network"],"testnet-10");assert_eq!(data["unfunded"],true);
 let fixtures=data["fixtures"].as_array().unwrap();assert!(fixtures.len()>=30);
 for fixture in fixtures{
  ...
  for (i,t) in fixture["assets"].as_array().unwrap().iter().enumerate(){
   let a=compile(t,string(&fixture["kind"]));let signature=ArtifactValue::Bytes(bytes(string(&fixture["signatures"][i])));
